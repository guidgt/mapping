...
//		"sSortColumn": "sorting_", /* Note that an int is postfixed for the sorting order */
		"sSortColumn": "", /* Note that an int is postfixed for the sorting order */
		"sSortJUIAsc": "",
		"sSortJUIDesc": "",
		"sSortJUI": "",
		"sSortJUIAscAllowed": "",
...
//				"sInfoFiltered": "(filtradas apartir de _MAX_ total entradas)",
				"sInfoFiltered": "",
				"sInfoPostFix": "",
				"sSearch": "Pesquisa:",
				"sUrl": "",
			...
			if ( oSettings.bAjaxDataGet )
			{
				...
				if ( oSettings.oFeatures.bFilter !== false )
				{
					...
					aoData.push( {"name": "bRegex",  "value": oSettings.oPreviousSearch.bRegex} );
                                        if(oSettings.oFeatures.todosRegistros){
                                            aoData.push( {"name": "todosRegistros",  "value": oSettings.oPreviousSearch.todosRegistros} );
                                        }
					for ( i=0 ; i<iColumns ; i++ )
					{
		...
		function _fnFeatureHtmlFilter ( oSettings )
		{
			...
			nFilter.innerHTML = oSettings.oLanguage.sSearch+sSpace+'<input type="text" />';
                        ...
                        if(oSettings.oFeatures.todosRegistros){
                            nFilter.innerHTML += sSpace+'<span>'+oSettings.oFeatures.todosRegistros+':</span><input style="float:right;margin-top: 1px;" type="checkbox" name="todosRegistros" value="" />';
                            var jqCheckFilter = $("input:checkbox", nFilter);

                        }
			
			var jqFilter = $("input:text", nFilter);
                        
			jqFilter.val( oSettings.oPreviousSearch.sSearch.replace('"','&quot;') );

                        if(oSettings.oFeatures.todosRegistros){
                            $(jqCheckFilter).click(function(){
                                _fnFilterComplete( oSettings, {
                                        "sSearch": jqFilter.val(),
                                        "bRegex":  oSettings.oPreviousSearch.bRegex,
                                        "bSmart":  oSettings.oPreviousSearch.bSmart,
                                        "todosRegistros":  $(this).attr("checked")
                                } );
                            });
                        }
			...
			jqFilter.keyup( function(e) {
				...
				/* Now do the filter */
                                if(jqCheckFilter != undefined){
                                    _fnFilterComplete( oSettings, {
                                            "sSearch": this.value,
                                            "bRegex":  oSettings.oPreviousSearch.bRegex,
                                            "bSmart":  oSettings.oPreviousSearch.bSmart,
                                            "todosRegistros":  $(jqCheckFilter).attr("checked")
                                    } );
                                }else{
                                    _fnFilterComplete( oSettings, {
                                            "sSearch": this.value,
		...
		function _fnFilterComplete ( oSettings, oInput, iForce )
		{
			...
			_fnFilter( oSettings, oInput.sSearch, iForce, oInput.bRegex, oInput.bSmart , oInput.todosRegistros);
			...
			for ( var i=0 ; i<oSettings.aoPreSearchCols.length ; i++ )
			{
				_fnFilterColumn( oSettings, oSettings.aoPreSearchCols[i].sSearch, i, 
					oSettings.aoPreSearchCols[i].bRegex, oSettings.aoPreSearchCols[i].bSmart,
                                        oSettings.aoPreSearchCols[i].todosRegistros );
			}
		 ...
		 */
		function _fnFilter( oSettings, sInput, iForce, bRegex, bSmart, todosRegistros)
		{
			var i;
			var rpSearch = _fnFilterCreateSearch( sInput, bRegex, bSmart );
			...
			oSettings.oPreviousSearch.todosRegistros = todosRegistros;
		}
...
//					alert( sAlert );
		...
		return this.each(function()
		{
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings.oFeatures, oInit, "todosRegistros" );
