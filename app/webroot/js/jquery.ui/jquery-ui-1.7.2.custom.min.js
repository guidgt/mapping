    ...
    j.toHide.filter(":hidden").each(j.complete).end().filter(":visible").animate(f,{
        step:function(k,l){
            if(l.prop=="height"){
                ...
                //Bloco alterado para IE
                if(isNaN(g)){
                    g=0;
                }
                //Fim - Bloco alterado para IE
...
_dialogDatepicker:function(input,dateText,onSelect,settings,pos){
    ...
    if(!inst){
        var id="dp"+(++this.uuid);
        this._dialogInput=$('<input type="text" id="'+id+'" size="1" style="position: absolute;top: -100px;z-index:1004"/>');
...
//var width=17;
var width=this._get(inst,"width");
if(cols>1){
    inst.dpDiv.addClass("ui-datepicker-multi-"+cols).css("width",(width*cols)+"em")
    }else{
    inst.dpDiv.removeClass("ui-datepicker-multi-2 ui-datepicker-multi-3 ui-datepicker-multi-4").width(width+"em")
    }
...
_selectDay:function(id,month,year,td){
    ...
    var inst=this._getInst(target[0]);
    var day = $("a",td).html().toLowerCase().split("<p>");

    inst.selectedDay=inst.currentDay=day[0];
