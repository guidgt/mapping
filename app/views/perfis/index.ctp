...
<div class="toolbar">
	<?php echo $html->link(__('Novo Perfil',true), 'add',array('class'=>'linkbutton linkbtn btn_add')); ?>	<a href="#" onclick="location.href='<?php echo $this->webroot;?>funcoes/deleteselected/'+selecionados()" class="linkbutton linkbtn btn_delete">Excluir Vários</a>
	...
	if($total!=1){
		...
		echo '<select style="height:22px;font-size:13px" onchange="location.href=\''. $this->webroot.'funcoes/index/page:\'+this.value+\''.$sort.$direction.'\'">';
		for($x=1;$x<=$total;$x++){
...
<div id="tabpanel">
    <ul>
        <li><a href="#tab1"><span><?php echo __("Listar Perfil",true) ?></span></a></li>
    </ul>
    <div id="tab1">
	...
	<tr<?php echo $class;?>>
						...
						<td align="center" style="padding: 0px 0px 0px 0px">
							<input type="checkbox" class="chk" value="<?=$perfil['Perfil']['id']?>">
						</td>
							<td>
			<?php echo $perfil['Perfil']['id']; ?>
		</td>
		<td>
			<?php echo $html->link($perfil['Perfil']['nome'], array('action'=>'view', $perfil['Perfil']['id']), array('class'=>'ctxmenu','id'=>$perfil['Perfil']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
