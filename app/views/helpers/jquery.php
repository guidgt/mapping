...
//    function init_date($local = "en", $start_date = "2000-01-01") {
    function init_date($local="pt-br", $start_date = "01/01/2010") {
        return $this->output('
                <script type="text/javascript" charset="utf-8">
                    $(function(){
...
//    function config_date($local="en", $start_date = "2000-01-01") {
    function config_date($local="pt-br", $start_date = "01/01/2010") {
    ...
    function dateTimeFormate($dateString, $hours = true) {
        if(!empty($dateString)) {
            $data = explode(' ',$dateString);
            $data[0] =  implode(preg_match("~\/~", $data[0]) == 0 ? "/" : "-", array_reverse(explode(preg_match("~\/~", $data[0]) == 0 ? "-" : "/", $data[0])));
            if($hours)
